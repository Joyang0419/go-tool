...
type THandler[TSO any, TVO any] func(ctx context.Context, so TSO) (vo TVO, err error)

type TAPI[TSO, TVO any] struct {
	method         string
	path           string
	handler        THandler[TSO, TVO]
...
func (receiver *TAPI[TSO, TVO]) Handler(handler THandler[TSO, TVO]) *TAPI[TSO, TVO] {
...
func (receiver *TAPI[TSO, TVO]) End(engine *gin.Engine) {
	if lo.IsNil(receiver.handler) {
		panicMsg := fmt.Sprintf("TAPI.End handler is nil, path: %s, method: %s", receiver.path, receiver.method)
		panic(panicMsg)
	}
	...
	mainHandlerFn := func(c *gin.Context) {
		...
		if err != nil {
			slog.ErrorContext(
				c.Request.Context(), "TAPI.mainHandlerFn.error: receiver.handler",
				slog.Any("error", err),
				slog.String("method", receiver.method),
				slog.String("path", receiver.path),
			)
